...
namespace MD5
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        public async void CheckLogin()
        {
            try
            {
                ...
                if (file.Length <= 0)
                {
                    ...
                    {
                        MessageDialoGG("Логин должен содержать хотя бы 6 символов");
                        return;
                    }
                }
                List<User> u;
        ...
        private void finalunlockcheck(object sender, RoutedEventArgs e) => CheckLogin();
        public async void create()
        {
            try
            {
                StorageFolder f = ApplicationData.Current.LocalFolder;
                StorageFile f1 = await f.CreateFileAsync("MD5Account.xml", CreationCollisionOption.OpenIfExists);
                FileInfo info = new FileInfo(f1.Path);
                List<User> u;
                XmlSerializer x = new XmlSerializer(typeof(List<User>));
                Stream s = await f1.OpenStreamForReadAsync();
                ...
                x.Serialize(s, u);
                s.Close();
                var yes = new MessageDialog("Входите");
                await yes.ShowAsync();
            }
            catch { }
        }
        private void VhodGo(object sender, TappedRoutedEventArgs e) => this.Frame.Navigate(typeof(Vhod));
        //Методы для удобного перехода в окна
        private void adminDebug_Click(object sender, RoutedEventArgs e) => Frame.Navigate(typeof(Admin));
        private void prepodDebug_Click(object sender, RoutedEventArgs e) => Frame.Navigate(typeof(Prepodavatel));
        private void userDebug_Click(object sender, RoutedEventArgs e) => Frame.Navigate(typeof(Student));
